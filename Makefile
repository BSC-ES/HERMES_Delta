...
# Paths to the HERMES_D software
#--------------------------------------------------------------------------------------
...
CONDA_ENV_DIR ?= "."
CONDA_ENV_PATH=$(CONDA_ENV_DIR)/HERMES_D_v$(version)
#--------------------------------------------------------------------------------------
# If the installation is for development, the name of the user will be added to the end of
# the name of the environment
create_env_for_dev: CONDA_ENV_PATH=$(CONDA_ENV_DIR)/HERMES_D_$(USER)
set_env_vars_for_dev: CONDA_ENV_PATH=$(CONDA_ENV_DIR)/HERMES_D_$(USER)
install_software_for_dev: CONDA_ENV_PATH=$(CONDA_ENV_DIR)/HERMES_D_$(USER)
full_installation_for_dev: CONDA_ENV_PATH=$(CONDA_ENV_DIR)/HERMES_D_$(USER)
...
install_software:
	# Install NES
	conda run -p $(CONDA_ENV_PATH) python -m pip install $(HERMES_PROJECT_PATH)/hermes_d/sources/NES
	# Install HERMES_D
...
install_software_for_dev:
	# Install NES
	conda run -p $(CONDA_ENV_PATH) python -m pip install -e $(HERMES_PROJECT_PATH)/hermes_d/sources/NES
	# Install HERMES_D
...
run_import_test:
	conda run -p $(CONDA_ENV_PATH) pytest $(HERMES_PROJECT_PATH)/hermes_d/tests/test_imports.py

temporal_unit_testing:
	conda run -p $(CONDA_ENV_PATH) pytest $(HERMES_PROJECT_PATH)/hermes_d/tests/test_temporal.py
	conda run -p $(CONDA_ENV_PATH) pytest $(HERMES_PROJECT_PATH)/hermes_d/tests/test_temporal_flexible.py

speciation_unit_testing:
	conda run -p $(CONDA_ENV_PATH) pytest $(HERMES_PROJECT_PATH)/hermes_d/tests/test_speciation.py

vertical_unit_testing:
	conda run -p $(CONDA_ENV_PATH) pytest $(HERMES_PROJECT_PATH)/hermes_d/tests/test_vertical.py


run_tests: run_import_test temporal_unit_testing speciation_unit_testing vertical_unit_testing
