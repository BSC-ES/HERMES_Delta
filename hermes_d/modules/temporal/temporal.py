...
class TemporalDelta(object):
    ...
    def _rebalance_weekly(self, unique_year_month_pairs, utc_times, rebalanced_weekly_factors,
        ...
        for year, month in unique_year_month_pairs:
            ...
            rebalanced_factors = self.__rebalance_weekly_profile__(
                weekly_profile=profiles["weekly"].loc[week_profile],
                year=year,
                month=month,
                profile_info=profile_info,
                do_holidays=self.holidays,
    ...
    def __rebalance_weekly_profile__(weekly_profile: Series, year: int, month: int, profile_info,
                                     do_holidays: bool, holiday_regions=None):
        import holidays
        ...
        if do_holidays:
            ...
            regional_holidays = holidays.country_holidays(
                country_code, subdiv=holiday_regions[nuts2_code], years=[year])
            # Combine national and regional holidays for the month
            holidays_days = [day for day in days_in_month if day in national_holidays or day in regional_holidays]
            # Count occurrences of each weekday, treating holidays as Sundays (6)
            weekday_counts = days_in_month.to_series().apply(
                lambda day: 6 if day in holidays_days else day.weekday()
    ...
    def __is_holiday__(self, local_date, nuts2_code):
        ...
        national_holidays = holidays.country_holidays(country=nuts2_code[:2].upper())
        regional_holidays = holidays.country_holidays(country=nuts2_code[:2].upper(),
                                                      subdiv=self.holiday_regions[nuts2_code])
