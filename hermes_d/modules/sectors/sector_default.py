...
class Sector(object):
    ...
    def __init__(self, sector_name, grid, clip, only_horizontal=False, data_paths=None, proxies_paths=None,
        ...
        if self.profiles_paths["individual_speciation_file"]:
            self.sector_info, self.point_sources, self.species_list, self.speciation_mapping = (
                SpeciationDelta(
        ...
        if not only_horizontal:
            ...
            self.sector_info, self.vertical_factors = (
                VerticalDelta(
                    profiles_paths=self.profiles_paths,
                    sector_info=self.sector_info,
                    output_layers=self.grid.lev['data']
                ).initialize())
