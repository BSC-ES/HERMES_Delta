...
class Nes(object):
            ...
            if len(self._full_lat["data"].shape) == 1:
                ...
                if self._full_lon_bnds is not None:
                    if global_limits["x_min"] < 0:
                        self._full_lon_bnds["data"] = concatenate(
                            (
                                self._full_lon_bnds["data"][global_limits["x_min"]:, :],
                ...
                if global_limits["x_min"] < 0:
                    self._full_lat["data"] = concatenate(
                        (
                            self._full_lat["data"][
                                global_limits["y_min"]:global_limits["y_max"], global_limits["x_min"]:, ],
                            self._full_lat["data"][
                                global_limits["y_min"]:global_limits["y_max"], :global_limits["x_max"], ],
                        ),
                        axis=1,
                    )
                    ...
                    self._full_lat["data"] = self._full_lat["data"][
                        global_limits["y_min"]:global_limits["y_max"], global_limits["x_min"]:global_limits["x_max"],]
                    ...
                    if idx_first != idx_last:  # To avoid single time step statistic
                        if idx_last != len(day_list):
                            if op == "mean":
                                ...
                                data_aux = nanmean(var_info["data"][idx_first:idx_last + 1, :, :, :], axis=0)
                            elif op == "nanmax":
                                data_aux = nanmax(var_info["data"][idx_first:idx_last + 1, :, :, :], axis=0)
                            elif op == "nanmin":
                                data_aux = nanmin(var_info["data"][idx_first:idx_last + 1, :, :, :], axis=0)
                            else:
                            ...
                            if op == "mean":
                                ...
                                data_aux = nanmean(var_info["data"][idx_first:, :, :, :], axis=0)
                            elif op == "nanmax":
                                data_aux = nanmax(var_info["data"][idx_first:, :, :, :], axis=0)
                            elif op == "nanmin":
                                data_aux = nanmin(var_info["data"][idx_first:, :, :, :], axis=0)
                            else:
                                raise NotImplementedError(f"Statistic operation '{op}' is not implemented.")
                            aux_time_bounds.append(
    ...
    def get_totals(self) -> dict:
        ...
        global_totals = self.comm.bcast(global_totals if self.rank == 0 else None, root=0)
    ...
    def _get_cell_measures_values(self, cell_measures_info):
        ...
        for cell_measures_var in cell_measures_info.keys():
            ...
            if coordinate_len == 1:
                ...
                raise NotImplementedError("The coordinate has wrong dimensions: {dim}".format(dim=values["data"].shape))
                ...
                if "strlen" in var_dims:
                    ...
                    for lat_n in range(data.shape[0]):
                        for lon_n in range(data.shape[1]):
                            data_aux[lat_n, lon_n] = (
                                "".join(data[lat_n, lon_n].tobytes().decode("ascii").replace("\x00", "")))
                        ...
                        for lev_n in range(data.shape[1]):
                            for lat_n in range(data.shape[2]):
                                for lon_n in range(data.shape[3]):
                                    data_aux[time_n, lev_n, lat_n, lon_n] = (
                                        "".join(data[time_n, lev_n, lat_n, lon_n].tobytes().decode("ascii").replace(
                                            "\x00", "")))
                    data = data_aux
                else:
                    raise NotImplementedError(
