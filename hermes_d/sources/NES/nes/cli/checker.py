...
def min_max_check(
    ...
    if not config.is_file():
        raise IsADirectoryError(
            f"Provided YAML configuration file {config} is not a file"
        )
    if not config.exists():
        raise FileExistsError(
            f"Provided YAML configuration file {config} does not exist."
        )

    # Open YAML file
    with open(config, "r") as config_file:
        logger.info(f"Opening YAML configuration file: {config}")
        try:
            config = yaml.safe_load(config_file)
        except Exception as e:
            raise yaml.YAMLError(
                f"Something went wrong during the opening of {config}: {e}"
            )
    ...
    for var in params.keys():
        ...
        if var not in nc_variables:
            logger.warning(
                f"Variable '{var}' provided in configuration file was not found in NetCDF - "
        ...
        if check_min(bounds=bounds, nc_bounds=nc_bounds):
            ...
            message = (f"Variable '{var}' min value {nc_bounds.min} is not in the expected range "
        ...
        if check_max(bounds=bounds, nc_bounds=nc_bounds):
            ...
            message = (f"Variable '{var}' max value {nc_bounds.max} is not in the expected range "
        ...
        if check_min_abs(bounds=bounds, nc_bounds=nc_bounds):
            ...
            message = (f"Variable '{var}' min value {nc_bounds.min} is below the expected "
        ...
        if check_max_abs(bounds=bounds, nc_bounds=nc_bounds):
            ...
            message = (f"Variable '{var}' max value {nc_bounds.max} is above the expected "
