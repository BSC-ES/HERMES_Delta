...
def _add_check_min_max_subparser(subparsers):
    ...
    check_parser = subparsers.add_parser(
        "check_min_max", help="Check min and max values of variables in a NetCDF file against a YAML configuration"
    )
...
def _add_nc2geostructure_subparser(subparsers):
    ...
    geo_parser = subparsers.add_parser(
        "nc2geostructure", help="Convert NetCDF to geospatial structure (GeoJSON, shapefile) (TESTING PHASE)")
    ...
    geo_parser.add_argument(
        ...
        type=int,
        default=0,
        help="Vertical level index to extract (default: 0)",
    )
...
def _add_interpolate_subparser(subparsers):
    ...
    general.add_argument("-o", "--output_file", required=True, help="Path to output NetCDF file")
...
def _add_check_subparser(subparsers):
    ...
    check_parser.add_argument("-l", "--var_list", nargs='+', default=None,
...
def _add_reorder_subparser(subparsers):
    ...
    reorder_parser.add_argument("-o", "--output_file", required=True, help="Output NetCDF file path")
...
def _add_nc2rline_subparser(subparsers):
    ...
    rline_parser.add_argument(
        "-i", "--input_file", required=True, help="NetCDF file to transform to RLINE format.")
    rline_parser.add_argument(
        "-o", "--output_file", required=True,
        help="Output CSV file path for RLINE emissions. If the filename includes the placeholder '<pollutant>', "
             "it will be replaced with each pollutant name, producing one CSV per variable. "
             "Otherwise, the variable name will be appended before the .csv extension.")
    rline_parser.add_argument(
        "-g", "--geometry", required=True, help="Geometry file (e.g., GeoJSON or Shapefile) with roads.")
    rline_parser.add_argument(
        "--crs", help="Coordinate Reference System to use for reprojection (e.g., 'EPSG:4326' or a PROJ string). "
                      "If not provided, the CRS from the geometry file will be used.")
    rline_parser.add_argument(
        "--output_geometry_file", help="Output CSV file path for RLINE-compatible geometry.", default=None)
    rline_parser.add_argument(
        "--index_column", help="Column name in the geometry to use as index.", default=None)
    rline_parser.add_argument(
        "--weight_column", help="Column name in the geometry to use as weight.", default=None)
    rline_parser.add_argument(
        "--weight_file", help="Auxiliary weight file (read if exists, write otherwise).", default=None)
    rline_parser.add_argument(
        "-l", "--var_list", nargs='+',
        help="List of NetCDF variables to include. If not provided, all variables will be used.", default=None)
...
def main():
    ...
    parser.add_argument(
        "--no-log", action="store_true", help="Suppress logging messages (only errors will be shown).")
    ...
    try:
        ...
        logger.error(f"[{args.command}] Rank {MPI.COMM_WORLD.Get_rank()}: NES critical error: {e}")
        logger.error(f"[{args.command}] Rank {MPI.COMM_WORLD.Get_rank()}: Traceback:\n{traceback.format_exc()}")
