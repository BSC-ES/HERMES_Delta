...
class Nes(object):
    ...
    def _str2char(self, data):
        ...
        data_aux = stringtochar(
            array([v.encode("ascii", "ignore") for v in data]).astype(
                "S" + str(self.strlen)
            )
        )
    ...
    def get_centroids_from_coordinates(self):
        ...
        for lat_ind in range(0, len(self.lat["data"])):
            for lon_ind in range(0, len(self.lon["data"])):
                centroids.append(
                    Point(self.lon["data"][lon_ind], self.lat["data"][lat_ind])
                )
    ...
    #     def _geojson_to_mbtile(
    #         mbtiles_file: str,
    #         geojson_file: str,
    #         min_zoom: int = 5,
    #         max_zoom: int = 12,
    #         max_size: int = 10000000000,
    #         max_features: int = 10000000000,
    #     ):
    #         """Converts a geojson file to an mbtile using gdal and the mbtile driver
    #     def __set_agg_tiles_hash(mbtiles_path: str) -> str:
    ...
    #         ----------
    #         mbtiles_file:str
    #             File path to the mbtile file to be created
    #         geojson_file:str
    #             File path to the geojson file converted from the netcdf
    #         min_zoom:int, optional
    #             Minimum zoom level at which tiles are generated, by default 5
    #         max_zoom:int, optional
    #             Maximum zoom level at which tiles are generated, by default 12
    #         max_size:int, optional
    #             Maximum size of a tile in bytes (after compression). If a tile is greater than this threshold,
    #             features will be written with reduced precision, or discarded, by default 10000000000
    #         max_features:int, optional
    #             Maximum number of features per tile, by default 10000000000
    #         """
    ...
    #             destNameOrDestDS=mbtiles_file,
    #             srcDS=geojson_file,
    #             options=opts,
    #         )
    #         # Recover resources held by dataset, remove file locks, equivalent to GDALClose()
    #         ds = None

    #     @njit(fastmath=True, cache=True, parallel=True)
    #     def _filter_by_index_sum(data_arrays: list[ndarray], threshold: float = 1e-12):
    #         """
    #         Filters a list of 1D arrays (same length) by summing values at each index across all arrays and applying
    #         a threshold.
    #         Essentially serves to filter out cells that have no data in them.
    ...
    #     if self.parallel_method != "T":
    #         raise (
    #             f"Warning, {self.parallel_method} incompatible with 'to_mbtiles', we can only parallelize across the
    #             time domain."
    #         )

    #     # Extract sectors available
    #     sectors = self.variables.keys()

    #     if not os.path.exists(mbtiles_path):
    #         # Create output directory if it doesn't exist
    #         os.makedirs(mbtiles_path, exist_ok=True)

    #     if geostructure is not None:
    ...
    #         # Select time slice
    #         sliced_data = self.sel_time(date, inplace=False)
    #         # Flatten sector data for each sector into a single array
    #         sector_arrays = [
    #             sliced_data.variables[sector]["data"].flatten() for sector in sectors
    #         ]
    #         # Filter 0s and near 0 values
    #         filtered_arrays, keep_mask = _filter_by_index_sum(sector_arrays)
    #         # Filter the shapefile DataFrame itself
    #         sliced_data.shapefile = sliced_data.shapefile[keep_mask].reset_index(
    #             drop=True
    #         )
    #         # Boolean mask of which indices to keep
    #         sliced_data._keep_mask = keep_mask
    #         for i, sector in enumerate(sectors):
    #             sliced_data.shapefile[sector] = filtered_arrays[i]
    ...
    #             tempfile.gettempdir(), f"temp_geojson_{self.rank}_{date_part}.geojson"
    #         )
    #         # Convert to shapefile
    #         sliced_data.to_shapefile(geojson_file, time=date, lev=0)

    #         # Assemble path to mbtiles file
    #         mbtiles = os.path.join(
    #             mbtiles_path, f"daily_{mbtiles_file}_{date_part}.mbtiles"
    #         )
    #         print(
    #             f"\tProcessing daily_{mbtiles_file}_{date_part}.mbtiles file, date: {date_part}",
    #             flush=True,
    #         )
    #         # Convert geojson to mbtile
    #         _geojson_to_mbtile(mbtiles, geojson_file)
    #         digest = __set_agg_tiles_hash(mbtiles)
