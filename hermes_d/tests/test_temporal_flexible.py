...
class TestTemporalDelta(unittest.TestCase):
    ...
    def test_flat_profile(self):
        ...
        result = self.temporal_delta.__rebalance_weekly_profile__(
            weekly_profile, year, month, profile_info, do_holidays=False
        )
    ...
    def test_rebalance_weekly_profile_with_mocked_holidays(self):
        ...
        self.temporal_delta.holiday_regions = {"ES51": "CT"}
        ...
        holidays = [Timestamp(datetime(2023, 1, 6).date())]
        weekday_counts = (
    ...
    def test_different_timezones(self):
        ...
        # temporal_delta = TemporalDelta(None, None, {}, time_steps, False)
    ...
    def test_empty_seie_both(self):
        ...
        with self.assertRaises(ValueError):
            TemporalDelta(
                pd.DataFrame({}), None, None, None, False, daily_consistency=False
            )
    ...
    def test_leapyear_profiles_nonleapyear_year(self):
        ...
        result = self.temporal_delta._check_temporal_mapping(
    ...
    def test_column_sum_mismatch(self):
        # 5x73 =365, but the profiles only have 73 cols. should raise an error if daily_consistency is True,
    ...
    def test_extract_temp_factors_leap_profiles_nonleap_year(self):
        # leap year profiles, but the year of the simulation is a non-leap year.
        self.temporal_delta.isleap_profiles = (
            True  # Set to True to indicate leap year profiles
        )

        profiles = {
            "daily": pd.DataFrame(
                [[1] * 366],
                columns=[str(i) for i in list(range(1, 367))],
                index=["D_0001"],
            ),
            "hourly": pd.DataFrame(
                [[1] * 24],
        ...
        profiles["hourly"] = profiles["hourly"].div(
            profiles["hourly"].sum(axis=1), axis=0
        )
        ...
        # scenario 1: leap year profile, but the year of the simulation is a nonleap year
        time_steps = pd.date_range(
            start="2023-06-15 11:00:00", end="2023-06-15 14:00:00", freq="h", tz="UTC"
        )  # 365-day year
        self.temporal_delta.time_step_list = time_steps
        ...
        utc_times = pd.DataFrame(
            {
                "year": time_steps.year.tolist(),
                "weekday": [0] * len(time_steps),
                "day": [0] * len(time_steps),
                "hour": [0] * len(time_steps),
                "day_of_year": time_steps.dayofyear.tolist(),
            }
        )

        self.temporal_delta._populate_inconsistent_time_step_list(utc_times)

        _, res_factors = self.temporal_delta._extract_temp_factors_leapyears(
            profile_id="D_0001",
        ...
        daily_factors, _ = res_factors

        for a, b in zip(daily_factors, [1 / 365] * len(time_steps)):
            self.assertAlmostEqual(a, b, places=12)

    def test_extract_temp_factors_leap_profiles_mixed_year(self):
        # leap year profiles, but the year of the simulation is a non-leap year.
        self.temporal_delta.isleap_profiles = (
            True  # Set to True to indicate leap year profiles
        )
        ...
        # scenario 1: start with non leap year, transtion into leap year
        time_steps = pd.date_range(
            start="2023-12-31 00:00:00", end="2024-01-01 01:00:00", freq="h", tz="UTC"
        )  # 365-day year
        self.temporal_delta.time_step_list = time_steps
        self.temporal_delta.set_inconsistent_time_steps(time_steps)
        ...
        # scenario 2: start with leap year, transtion into non-leap year
        time_steps = pd.date_range(
            start="2024-12-31 00:00:00", end="2025-01-01 01:00:00", freq="h", tz="UTC"
        )
        self.temporal_delta.time_step_list = time_steps
        self.temporal_delta.set_inconsistent_time_steps(time_steps)
        ...
        for a, b in zip(daily_factors, [1 / 366] * len(time_steps)):
            self.assertAlmostEqual(a, b, places=12)

    def test_extract_temp_factors_nonleap_profiles_leap_year(self):
        # non-leap year profiles, but the year of the simulation is a leap year.
        self.temporal_delta.isleap_profiles = (
            False  # Set to False to indicate non-leap year profiles
        )

        profiles = {
            "daily": pd.DataFrame(
                [[1] * 365],
                columns=[str(i) for i in list(range(1, 366))],
                index=["D_0001"],
            ),
            "hourly": pd.DataFrame(
                [[1] * 24],
    ...
    def test_extract_temp_factors_nonleap_profiles_mixed_year(self):
        # non leap year profiles, but the year of the simulation is a leap year.
        self.temporal_delta.isleap_profiles = (
            False  # Set to False to indicate non leap year profiles
        )
        ...
        # scenario 1: start with leap year, transtion into non leap year
        time_steps = pd.date_range(
            start="2024-12-31 00:00:00", end="2025-01-01 01:00:00", freq="h", tz="UTC"
        )
        self.temporal_delta.time_step_list = time_steps
        ...
        # scenario 2: start with non leap year, transtion into leap year
        time_steps = pd.date_range(
            start="2023-12-31 00:00:00", end="2024-01-01 01:00:00", freq="h", tz="UTC"
        )
        self.temporal_delta.time_step_list = time_steps
    ...
    # TODO: add unit test related to inconsistent_time_steps for different time zones.
    # to make sure that it follows the utc times rather than local times.
