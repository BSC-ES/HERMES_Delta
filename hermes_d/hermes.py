...
# from hermesv3_d.config import get_memory_stamp, record_memory, finalize_mem_log
...
class HermesDelta(object):
    ...
    # @profile(stream=open(f"log_mem_HERMES_{str(MPI.COMM_WORLD.Get_rank()).zfill(3)}.txt", mode="w+"))
    def __init__(
        self,
        config: ConfigDelta,
        ...
        configure_logger(
            log_path=str(
                os.path.join(
                    ...
                    "logs",
                    os.path.basename(self.config.get_output_name(self.start_date)),
                )
            ).replace(".nc", "_<rank>.log"),
        ...
        if (
            ...
            if self.master:
                raise AttributeError(
                    "ERROR: Global domain is not available for {0} output model.".format(
    ...
    def make_sector_list(self) -> List[Sector]:
        ...
        sector_list = []
        for sector_name, sector_info in self.options.sector_information[
            "SECTORS"
        ].items():
            ...
            sector_info["Proxies"]["Proxy_aux_path"] = os.path.join(
                self.options.auxiliary_files_path,
                sector_name,
                "{0}_proxy.nc".format(sector_name),
            )

            # Add common speciation file to "Profiles"
            sector_info["Profiles"]["individual_speciation_file"] = (
                self.options.individual_speciation_file
            )
            sector_info["Profiles"]["spec_aux_path"] = os.path.join(
                self.options.auxiliary_files_path,
                sector_name,
                "{0}_speciation_mapping.txt".format(sector_name),
            )
            # Add nuts2_info to "Profiles"
            sector_info["Profiles"]["nuts2_info"] = self.options.nuts2_info

            input_pollutants = (
                sector_info["Pollutants"]
                if "Pollutants" in sector_info.keys()
                else None
            )
    ...
    def prepare_inventories(self) -> None:
        ...
        for num, sector in enumerate(self.sector_list):
            log_message(
                "Processing {0} sector. ({1}/{2}):".format(
                    sector.name, num + 1, len(self.sector_list)
                ),
                level=2,
            )
            ...
            if not sector.only_horizontal:
                ...
                sector.sector_info = distribute_seie_vertically(
                    sector.vertical_factors,
                    sector.sector_info,
                    sector.grid.lev["data"],
                    sector.pollutants,
                )
                if sector.point_sources is not None:
                ...
                if sector.point_sources is not None:
                    ...
                    sector.point_sources = sector.point_sources.drop(
                        columns=["height", "plume_rise_factor"]
                    )
            ...
            record_time(
                f"{sector.name}", "PointSourcesAllocation", get_time_stamp() - st_time
            )

        record_time(
            "EmissionInventory", "prepare_inventories", get_time_stamp() - st_time
        )
        return None
    ...
    def prepare_output(self) -> Nes:
        ...
        if self.only_horizontal:
            result.set_levels(
                {"data": np.array([0], dtype=precision), "units": "m", "positive": "up"}
            )
        else:
            result.set_levels(
                {
                    "data": np.array(
                        self.options.vertical_description, dtype=precision
                    ),
                    "units": "m",
                    "positive": "up",
                }
        ...
        if self.out_format == "CMAQ":
            if self.options.output_attributes is not None:
                global_attributes = pd.read_csv(
                    self.options.output_attributes, sep=",", index_col="attribute"
                )
                for attr_name in [
                    "EXEC_ID",
                    "FTYPE",
    ...
    def main(self) -> Optional[datetime]:
        ...
        if self.first_time:
            ...
            for i_time, time_step in enumerate(self.time_step_list):
                log_message(
                    "Creating {0} data ({1}/{2})".format(
                        time_step, i_time + 1, len(self.time_step_list)
                    ),
                    level=2,
                )
        ...
        finalize_time_log(
            ...
            ).replace(".nc", f"_Times_{str(self.size).zfill(4)}.csv")
        )
...
def run() -> None:
    ...
    try:
        ...
        log_message(
            f"Process {MPI.COMM_WORLD.Get_rank()}: Critical error detected {e}, aborting MPI.",
            level=9,
        )
        log_message(
            f"Process {MPI.COMM_WORLD.Get_rank()}: Traceback:\n{traceback.format_exc()}",
            level=9,
        )
